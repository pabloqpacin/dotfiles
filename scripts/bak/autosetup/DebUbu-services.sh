...
function install_mysql_wb_ubupop {
    if ! command -v mysql &>/dev/null; then
        ...
        read -r -p "== Expect an interactive menu. Defaults are fine. Press any key. == " null
        sudo dpkg -i mysql-apt-config_0.8.26-1_all.deb
        echo ""; read -r -p "== Expect an interactive menu. Set a strong root password. Press any key. == " null
...
function config_mysql {
    if command -v mysql &>/dev/null && [[ ! -e ${mysql_conf} ]]; then
        ...
        while [[ ${password_user} != "${password_check}" ]]; do
            read -r -sp "Enter the password for the new user: " password_user; echo ""
            read -r -sp "Re-enter the password: " password_check; echo ""
...
function install_apache_php_wordpress {
    if [[ ! -d /srv/www ]]; then
        ...
        curl https://wordpress.org/latest.tar.gz | sudo -u www-data tar zx -C /srv/www || true
    else
...
function config_mysql_wordpress {
    if [[ ! -e ${wp_config_file} ]]; then
        ...
        while [[ ${password_wordpress} != "${password_wp_check}" ]]; do
            read -r -sp "Enter the password for the new user: " password_wordpress; echo ""
            read -r -sp "Re-enter the password: " password_wp_check; echo ""
...
Your Email: whoami@test.com"
        ...
        read -r -p "Ready? Press any key. " null
...
read -r -p "Do you want to set up *mysql*? [Y/n] " opt
if [[ ${opt} == "Y" || ${opt} == "y" || ${opt} = "" ]]; then
    setup_mysql='1'
fi

read -r -p "Do you want to set up *wordpress*? [Y/n] " opt
if [[ ${opt} == "Y" || ${opt} == "y" || ${opt} = "" ]]; then
    setup_wordpress='1'
fi
