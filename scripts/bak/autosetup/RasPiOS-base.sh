...
install_base_cargo() {
    ...
    if ! command -v bat &>/dev/null || ! command -v eza &>/dev/null || ! command -v delta &>/dev/null || ! command -v zoxide &>/dev/null; then
        ...
        read -r -p "Build cargo packages [bat eza git-delta zoxide] if don't exist? [Y/n] (~10 min.) " opt
...
install_base_go() {
    ...
    source "${HOME}/dotfiles/zsh/golang.zsh"
...
setup_nvim__npm() {
    ...
    if [[ ! -L ~/.config/nvim ]]; then
        ...
        read -r -p "Skip messages with Enter, then do :so :PackerUpdate :qa ... " null
        [[ -n "${null}" || -z "${null}" ]] && nvim ~/.config/nvim/lua/pabloqpacin/packer.lua
    fi
