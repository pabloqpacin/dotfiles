...
#
# Feb 2022 TOURE A. KARIM -- https://github.com/trabdlkarim/parrot-zsh-theme
...
PROMPT="
%F{red}┌─[%{$reset_color%}%f%F{yellow}%D{%H:%M-%d/%m}%f%F\
%F{red}]─[%{$reset_color%}\
%(#,%{$bg[yellow]%}%{$fg[black]%}%n%{$reset_color%},%{$fg[cyan]%}%n)\
%{$reset_color%}@\
%{$fg[green]%}%m\
%{$reset_color%}%F{red}]─[%{$reset_color%}\
%{$terminfo[none]$fg[magenta]%}%~%{$reset_color%}\
%F{red}]%{$reset_color%}\
${hg_info}\
${git_info}\
${svn_info}\
${venv_info}\
 \
$exit_code
%{$terminfo[none]$fg[red]%}└─╼ %{$reset_color%}\
%(?:%F{yellow}$ :%F{red}$ )%{$reset_color%}"
